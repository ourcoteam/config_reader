...
Future<Map<String, String>?> versions() async {
  ...
  final lastAndroidVersionRaw = pubFileYaml
      .readAsLinesSync()
      .firstWhere((element) => element.contains('version:'));
  final lastAndroidVersion =
      lastAndroidVersionRaw.trim().replaceFirst('version: ', '');
  ...
  final lastAndroidVersionPrefixString = lastAndroidVersionStringSegments
      .take(lastAndroidVersionStringSegments.length - 1)
      .join('.');
  ...
  try {
    final lastIosVersionNumberStringRaw = infoPlistLines
        .elementAt(infoPlistLines.indexWhere(
                (element) => element.contains('CFBundleShortVersionString')) +
            1)
        .trim()
        .replaceFirst('<string>', '')
        .replaceFirst('</string>', '');
    final lastIosVersionNumberStringSegments =
        lastIosVersionNumberStringRaw.split('.');
    final lastIosVersionNumberString = lastIosVersionNumberStringSegments.last;
    lastIosVersionPrefixString = lastIosVersionNumberStringSegments
        .take(lastIosVersionNumberStringSegments.length - 1)
        .join('.');
  ...
  return {
    "android":
        '$lastAndroidVersionPrefixString.$newAndroidVersionNumber+${newAndroidVersionNumber + 1}',
